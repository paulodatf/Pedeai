...
async function inicializar() {
    ...
    // Segurança: no iPhone sem internet o Firebase pode travar sem lançar erro.
    // Este timeout garante que o splash sempre seja removido em até 6s.
    ...
    grid.querySelectorAll('video').forEach(video => {
        ...
        removerSplash();
        window.IosOverlayManager?.hideAll();
        if (todosProdutos.length === 0) mostrarEstadoOffline();
    }
...
async function renderizarProdutos(opcoes = {}) {
    ...
    if (filtrados.length === 0 && !navigator.onLine) {
...
window.addEventListener('pageshow', (event) => {
    ...
    function finalizarCarga() {
        ...
        inicializarArialProdutos();
        renderizarFiltros();
        // Só re-renderiza os produtos se não houver cache do DOM válido
        ...
        if (!domCacheAtual || !gridAtual || gridAtual.innerHTML.trim() === '') {
            ...
            // Restaura src nos vídeos que ficaram sem fonte após bfcache ou retorno
